...
export const modelRepository = {
  ...
  async create(model: Omit<Model, 'created_at' | 'updated_at'>): Promise<Model> {
    ...
    try {
      await conn.query(
        'INSERT INTO models (id, name, provider_id, model_identifier, supported_protocols, is_virtual, routing_config_id, expert_routing_id, enabled, model_attributes, compression_config, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [model.id, model.name, model.provider_id, model.model_identifier, model.supported_protocols || '["openai"]', model.is_virtual, model.routing_config_id, model.expert_routing_id || null, model.enabled, model.model_attributes, model.compression_config, now, now]
      );
