...
export function createDecisionsProxyHandler() {
  return async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const { virtualKey, virtualKeyValue, providerId, currentModel, modelResult, configResult } =
      pipelineResult.context;

    const body = request.body as any;
    if (!body || typeof body !== 'object' || Array.isArray(body)) {
      return reply.code(400).send(invalidDecisionsRequest('Request body must be a JSON object', 'body'));
    }
    if (body.state === undefined || body.state === null || body.state === '') {
      return reply.code(400).send(invalidDecisionsRequest('Missing required field: state', 'state'));
    }
    if (
      !body.questions ||
      ...
      return reply.code(400).send(invalidDecisionsRequest('Missing required field: questions (non-empty object)', 'questions'));
    }
    ...
    if (typeof modelAttributes.decisions_path === 'string' && modelAttributes.decisions_path) {
    ...
    try {
      ...
      const tokenCount = usage
        ? await calculateTokensIfNeeded(norm.totalTokens, request.body, response.body)
        : { promptTokens: 0, completionTokens: 0, totalTokens: 0 };

      const shouldLogBody = shouldLogRequestBody(virtualKey);
      const truncatedRequest = shouldLogBody ? truncateRequestBody(upstreamBody) : undefined;
      ...
      logApiRequestAsync({
        ...
        truncatedRequest: shouldLogBody ? truncateRequestBody(upstreamBody) : undefined,
        cacheHit: 0,
        ip: requestIp,
        userAgent: requestUserAgent,
        piiMaskedCount: 0,
