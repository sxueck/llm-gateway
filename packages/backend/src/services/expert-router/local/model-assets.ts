...
export function loadLocalClassifierAssets(): Promise<LoadedAssets> {
  if (state.status === 'ready' && state.assets) return Promise.resolve(state.assets);
...
async function doLoad(): Promise<LoadedAssets> {
  ...
  if (!existsSync(markerPath)) {
    throw new Error(`missing revision marker (${REVISION_MARKER_FILE}); run the download script`);
  }
  ...
  const intercept = parseNpy(readFileSync(resolve(MODEL_DIR, 'head_intercept.npy')));
  if (coef.shape.length !== 2 || coef.shape[1] !== 1024) {
    throw new Error(`unexpected head_coef shape: ${JSON.stringify(coef.shape)} (expected [C, 1024])`);
  }
  ...
  const labelsDoc = JSON.parse(readFileSync(resolve(MODEL_DIR, 'labels.json'), 'utf8'));
  ...
  const policy = parsePolicy(
    JSON.parse(readFileSync(resolve(MODEL_DIR, 'rejection_policy.json'), 'utf8'))
  );
...
function ensureTokenizerCompat(modelDir: string): void {
  ...
  doc.model.merges = merges.map((pair: any) =>
    Array.isArray(pair) ? pair.map((p: any) => String(p)).join(' ') : String(pair)
  );
...
function parsePolicy(raw: any): RejectionPolicy {  return {
