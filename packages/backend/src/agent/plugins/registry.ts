...
export function computeBundleDigest(bundle: PluginBundle): string {
  const canonical = canonicalJson({ manifest: bundle.manifest, files: bundle.files });
...
export function validatePluginBundle(bundle: PluginBundle): string[] {
  ...
  if (!parsed.success) {
    return parsed.error.issues.map((i) => `manifest: ${i.path.join('.') || '<root>'} ${i.message}`);
  }
  ...
  for (const [field, ref] of [
    ...
    if (!isSafeBundleFileRef(ref)) {
      ...
      if (!bundle.files[name]) errors.push(`${field}: referenced file "${name}" missing from bundle`);
    }
  }
  ...
  if (extraFiles.length > 0) {
    errors.push(`bundle contains files outside the allowed set: ${extraFiles.join(', ')}`);
  }
  ...
  if (unknownTools.length > 0) {
    errors.push(`tool_policy.allow contains tools not offered by the platform: ${unknownTools.join(', ')}`);
  }

  for (const root of manifest.workspace_policy.allowed_roots) {
    if (!root.startsWith('/workspace/')) {
      errors.push(`workspace_policy.allowed_roots entry "${root}" escapes the platform workspace root`);
    }
  }
  ...
  const fileItemRequired =
    outputSchema?.properties?.files?.items?.required;
...
// 启动时自校验：内置 fixture 必须通过发布级校验，否则拒绝注册。
...
for (const b of BUNDLES) {
  ...
  const key = `${b.manifest.id}@${b.manifest.version}`;
  if (registry.has(key)) {
    throw new Error(`duplicate plugin version ${key}`);
  }
  registry.set(key, b);
}

export function listPlugins(): { manifest: WorkerPluginManifest; digest: string }[] {
  return [...registry.values()].map((b) => ({ manifest: b.manifest, digest: computeBundleDigest(b) }));
}

export function resolvePlugin(id: string, version: string): ResolvedPlugin | undefined {
  const b = registry.get(`${id}@${version}`);
  if (!b) return undefined;
  const errors = validatePluginBundle(b);
  if (errors.length > 0) {
    throw new Error(`plugin ${id}@${version} failed runtime re-validation: ${errors.join('; ')}`);
  }
  return { ...b, digest: computeBundleDigest(b) };
}
