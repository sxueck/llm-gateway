...
describe("plugin registry", () => {
  it("computes a stable sha256 digest for a pristine bundle", () => {
    const b = validBundle();
    const digest = computeBundleDigest(b);
    expect(digest).toMatch(/^sha256:[0-9a-f]{64}$/);
    expect(computeBundleDigest(JSON.parse(JSON.stringify(b)))).toBe(digest);
  });
  ...
  it("rejects path traversal in role file references", () => {
    ...
    expect(validatePluginBundle(b).join(" ")).toContain(
      "unsafe or unknown bundle file reference",
    );
  });
  ...
  it("rejects role references to files missing from the bundle", () => {
    ...
    expect(validatePluginBundle(b).join(" ")).toContain(
      'referenced file "prompt.md" missing',
    );
  });
  ...
  it("rejects tools outside the platform allowlist", () => {
    ...
    expect(validatePluginBundle(b).join(" ")).toContain(
      "tools not offered by the platform",
    );
  });
  ...
  it("rejects workspace roots escaping /workspace", () => {
    ...
    expect(validatePluginBundle(b).join(" ")).toContain(
      "escapes the platform workspace root",
    );
  });
  ...
  it("rejects output schemas missing mandatory top-level or per-file fields", () => {
    ...
    expect(validatePluginBundle(b2).join(" ")).toContain(
      'file items must require "path"',
    );
  });
...
describe("snapshot path guards", () => {
  ...
  it("glob translation matches full paths and basenames", () => {
    ...
    expect(
      globToRegExp("node_modules/**").test("node_modules/pkg/index.js"),
    ).toBe(true);
  });
  ...
  it("blocks platform-forbidden paths while allowing regular sources", () => {
    ...
    expect(isForbiddenSnapshotPath("packages/backend/src/index.ts")).toBe(
      false,
    );
