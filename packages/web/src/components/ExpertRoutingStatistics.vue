    ...
    <n-space vertical :size="16">
      <n-grid :cols="3" :x-gap="12">
        ...
        <n-gi>
          <n-card size="small">
            <n-statistic
              :label="t('expertRouting.cleaningEfficiency')"
              :value="cleaningEfficiency"
            >
              ...
              <n-grid :cols="2" :x-gap="24">
                <n-gi>
                  <n-space vertical :size="8">
                    <div>
                      <n-text strong
                        >{{ t("expertRouting.classificationResult") }}:</n-text
                      >
                      {{ selectedLogDetail.classification_result }}
                    </div>
                    <div>
                      <n-text strong
                        >{{ t("expertRouting.selectedExpert") }}:</n-text
                      >
                      {{ selectedLogDetail.selected_expert_name }}
                    </div>
                    <div>
                      <n-text strong
                        >{{ t("expertRouting.classifierModel") }}:</n-text
                      >
                      {{ selectedLogDetail.classifier_model }}
                    </div>
                    <div>
                      <n-text strong
                        >{{ t("expertRouting.classificationTime") }}:</n-text
                      >
            ...
            <n-card size="small" class="log-detail-card collapsible-card">
              ...
              <div v-show="showOriginalRequest">
                <n-code
                  ...
                  :code="
                    JSON.stringify(selectedLogDetail.original_request, null, 2)
                  "
                  language="json"
            ...
            <n-card size="small" class="log-detail-card collapsible-card">
              ...
              <div v-show="showClassifierRequest">
                <n-code
                  ...
                  :code="
                    JSON.stringify(
                      selectedLogDetail.classifier_request,
                      null,
                      2,
                    )
                  "
            ...
            <n-card size="small" class="log-detail-card collapsible-card">
              ...
              <div v-show="showClassifierResponse">
                <n-code
                  ...
                  :code="
                    JSON.stringify(
                      selectedLogDetail.classifier_response,
                      null,
                      2,
                    )
                  "
...
function getSourceTagType(source?: string): NaiveTagType {
  if (source && source in ROUTE_SOURCE_META)
    return ROUTE_SOURCE_META[source as RouteSource].tag;
  return "default";
}
