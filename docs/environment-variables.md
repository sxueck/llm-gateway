...
| 变量 | 默认值 | 说明 |
| --- | --- | --- |
| `LOCAL_INTENT_CLASSIFIER` | 启用 | 设为 `off` / `disabled` / `0` / `false` 时关闭本地 ONNX 意图分类器：启动不加载模型（节省 ~615MB 内存），专家路由回退到 LLM 二次分类 / fallback，`/v1/intent/classify` 返回 `503 classifier_disabled`。详见 [README · 意图路由分类器](../README.md#意图路由分类器)。 |
| `EXPERT_ROUTING_MODEL_DIR` | `<工作目录>/model-assets/intent-router-zh-setfit-v1` | 本地 ONNX 分类器 artifacts 目录（由下载脚本填充）。 |
